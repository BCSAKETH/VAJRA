...
- [ ] **Real Console setup still needed** (cannot be done from here): (1) create the actual Catalyst Signal on `CaseMaster` INSERT, pointed at the `case_insert_signal` function; (2) set `VAJRA_APP_BASE_URL` and `INTERNAL_SIGNAL_SECRET` on that function's environment (must match the same `INTERNAL_SIGNAL_SECRET` value set on the main app); (3) once live, check the Signal actually fires with the payload shape this handler assumes — adjust `_extract_case_master_id` if not.
