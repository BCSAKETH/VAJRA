...
import {
  ResponsiveContainer, BarChart, Bar, LineChart, Line, XAxis, YAxis, Tooltip, CartesianGrid, LabelList, Cell,
} from "recharts";
...
// SAME all-district dataset the old standalone Reports screen used to fetch,
// just re-framed around one district instead of a flat 30-bar comparison.
...
/** §Part-G-district: real cross-district correlation. `district: null` means
 * no district is picked yet -- shows the full statewide comparison (every
 * district, no highlight). `district: "<name>"` narrows to "how does THIS
 * district compare to the state average", the same real dataset just
 * re-framed, not a second data source. */
export const DistrictDemographicPanel: React.FC<{
  ...
  const thisRow = district ? allDistricts?.find((r) => r.district === district) || null : null;
  ...
  return (
    <div className="space-y-4">
      {district ? (
        <>
          {/* This-district-vs-state-average correlation cards -- the actual
              "correlation" insight: not just this district's raw numbers,
              but how far from typical it sits. */}
        ...
        // Statewide default (no district picked yet): the aggregate figures
        // that used to live on the standalone Demographic Correlation
        // screen, same real dataset, no per-district framing possible yet.
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { label: "Avg. Crime Incidence", value: stateAvg?.crimeCount, unit: "" },
            { label: "Avg. Unemployment", value: stateAvg?.unemploymentRate, unit: "%" },
            { label: "Avg. Literacy Rate", value: stateAvg?.literacyRate, unit: "%" },
          ].map((m, i) => (
            <div key={i} className="glass-card p-3.5 border border-stone-850 space-y-1.5">
              <div className="text-[9.5px] font-mono uppercase tracking-widest text-stone-500">{m.label}</div>
              <div className="text-xl font-black text-stone-100 font-mono tabular-nums">
              ...
              <div className="text-[9px] text-stone-600 font-mono">across all districts</div>
            </div>
          ))}
        </div>
      )}

      {/* Cross-district correlation strip -- every OTHER district muted
          gray, the SELECTED one gold, sorted by crime volume so its rank
          among peers is immediately visible. With no district picked, every
          bar renders in the same neutral gold (nothing to contrast yet). */}
      <div className="glass-card p-4 border border-stone-850 space-y-2">
        <h3 className="text-[11px] font-black text-stone-200 uppercase tracking-wider font-mono flex items-center gap-1.5">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <Bar dataKey="crimeCount" radius={[4, 4, 0, 0]}>
                  {chartData.map((row, i) => (
                    <Cell key={i} fill={!district || row.district === district ? "#C79A4E" : "#3a352e"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      {/* Statewide-only: unemployment vs incidents line overlay across every
          district -- the correlation shape itself, only meaningful state-
          wide (a single district has nothing to correlate against). */}
      {!district && (
        <div className="glass-card p-4 border border-stone-850 space-y-2">
          <h3 className="text-[11px] font-black text-stone-200 uppercase tracking-wider font-mono">
            Unemployment Rate vs Incident Volume — Statewide
          </h3>
          {!allDistricts ? (
            <div className="h-52 flex items-center justify-center text-[10px] text-stone-500 font-mono">Loading...</div>
          ) : (
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={allDistricts}>
                  ...
                  <Line type="monotone" dataKey="unemploymentRate" stroke="#C79A4E" strokeWidth={2} name="Unemployment %" strokeDasharray="4 4" dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
